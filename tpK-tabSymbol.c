...
void ajouterVariable(char *identif){
	if(!existe(identif)){
		ajouterEntree(identif, contexte, typeVar, adresse_courant[curseur]++, 0);
	} else erreurFatale("Erreur : 2 variables avec le même identifiant\n");
}

int ajouterFonction(char *identif, int nb_params){
	if(!existe(identif)){
		ajouterEntree(identif, C_FONCTION, typeVar, adresse_courant[ADR_GLOB]++, nb_params);
		return 1;
	} else erreurFatale("Erreur : 2 fonctions avec le même identifiant\n");
	return 0;
}
